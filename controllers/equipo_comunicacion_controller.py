...
from models.equipo_comunicacion_model import Equipo
from views import equipo_comunicacion_view
...
def index():
    ...
    return equipo_comunicacion_view.list(equipos)
...
def create():
    if request.method == 'POST':
        try:
            ...
            if 'imagen' in request.files:
                ...
                if file.filename != '':
                    ...
                    if '.' in file.filename and file.filename.rsplit('.', 1)[1].lower() in allowed_extensions:
                        ...
                        return redirect(url_for('equipo.create'))
            ...
            equipo.save()
            flash('Equipo de Comunicacion creado exitosamente', 'success')
            return redirect(url_for('equipo.index'))
            
        except Exception as e:
            current_app.logger.error(f"Error al crear Equipo de Comunicacion: {str(e)}")
            flash('Error al crear el Equipo de Comunicacion', 'danger')

    return equipo_comunicacion_view.create()


#Edicion de equipos
@equipo_bp.route('/edit/<int:id>', methods=['GET', 'POST'])
def edit(id):
    equipo = Equipo.get_by_id(id)
    if request.method == 'POST':
        descripcion = request.form['descripcion']
        marca = request.form['marca']
        modelo = request.form['modelo']
        ...
        imagen = equipo.imagen  # Mantener la imagen existente por defecto
        if 'imagen' in request.files:
                file = request.files['imagen']
                if file.filename != '':
                    # Eliminar la imagen anterior si existe
                    if equipo.imagen:
                        old_image_path = os.path.join(current_app.config['UPLOAD_FOLDER'], equipo.imagen)
    ...
    return equipo_comunicacion_view.edit(equipo)



@equipo_bp.route('/imprimir')
def imprimir():
    ...
    try:
        ...
        title = Paragraph("REPORTE DE EQUIPOS DE COMUNICACIÓN", title_style)
        ...
        for v in equipos:
            ...
            data.append([
                ...
                v.marca,
                v.modelo,
                f"${v.costo_inicial:,.2f}",
                f"${v.valor_residual:,.2f}",
        ...
        def add_page_number(canvas, doc):
            ...
            canvas.drawString(inch, 0.5*inch, f"Total de Equipos de Comunicacion: {len(equipos)}")
        ...
        return send_file(
            buffer,
            as_attachment=True,
            download_name=f"reporte_equipos_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf",
...
@equipo_bp.route('/imprimir_confirmar')
def imprimir_confirmar():
    ...
    return redirect(url_for('equipo.imprimir'))  
    
# Eliminar vehículo
@equipo_bp.route('/delete/<int:id>')
def delete(id):
    equipo = Equipo.get_by_id(id)
    equipo.delete()
    return redirect(url_for('equipo.index'))
