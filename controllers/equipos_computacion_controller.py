...
def index():
    ...
    # Solo traer EQUIPOS DE COMPUTACIÓN del usuario actual
...
def create():
    if request.method == 'POST':
        try:
            ...
            if 'imagen' in request.files:
                ...
                if file.filename != '':
                    ...
                    if '.' in file.filename and file.filename.rsplit('.', 1)[1].lower() in allowed_extensions:
                        ...
                        return redirect(url_for('computadora.create'))
            ...
            current_app.logger.error(f"Error al crear Equipo: {str(e)}")
            flash('Error al crear un Equipo', 'danger')

    return equipos_computacion_view.create()


#Edicion de computadoras
@computadora_bp.route('/edit/<int:id>', methods=['GET', 'POST'])
def edit(id):
    ...
    if request.method == 'POST':
        ...
        imagen = computadora.imagen  # Mantener la imagen existente por defecto
        if 'imagen' in request.files:
                file = request.files['imagen']
                if file.filename != '':
                    # Eliminar la imagen anterior si existe
                    if computadora.imagen:
                        old_image_path = os.path.join(current_app.config['UPLOAD_FOLDER'], computadora.imagen)
    ...
    return equipos_computacion_view.edit(computadora)



@computadora_bp.route('/imprimir')
def imprimir():
    ...
    try:
        ...
        title = Paragraph("REPORTE DE EQUIPOS DE COMPUTACION", title_style)
        ...
        def add_page_number(canvas, doc):
            ...
            canvas.drawString(inch, 0.5*inch, f"Total de Equipos de Computacion: {len(computadoras)}")
        ...
        return send_file(
            buffer,
            as_attachment=True,
            download_name=f"reporte_computadoras_{datetime.now().strftime('%Y%m%d_%H%M')}.pdf",
...
@computadora_bp.route('/imprimir_confirmar')
def imprimir_confirmar():
    ...
    return redirect(url_for('computadora.imprimir'))  
    
# Eliminar vehículo
@computadora_bp.route('/delete/<int:id>')
def delete(id):
    computadora = Computadora.get_by_id(id)
    computadora.delete()
    return redirect(url_for('computadora.index'))
...
def incorporacion():
    ...
    total_paragraph = Paragraph(
        f"<b>Total de Computadoras:</b> {len(computadoras)} | "
...
def financiero():
    ...
    # Párrafo resumen debajo de la tabla
    total_paragraph = Paragraph(
        f"<b>Total de Equipos de Computación:</b> {len(computadoras)} | "
    ...
    def add_page_number(canvas, doc):
        ...
        canvas.setFont('Helvetica', 8)
        canvas.setFillColor(colors.HexColor('#7f8c8d'))
        canvas.drawString(inch, 0.5*inch, f"Total de Equipos de Computación: {len(computadoras)}")
        canvas.drawRightString(landscape(letter)[0] - inch, 0.5*inch, f"Página {doc.page}")
        canvas.setStrokeColor(colors.black)
        canvas.line(inch, inch, 3*inch, inch)
        canvas.drawString(inch, 0.75*inch, "Responsable del reporte")
